...
public class ServiceInstancesApiServiceImpl extends ServiceInstancesApiService {
	 ...
	 */
	@Override
	public Response serviceInstancesInstanceIdPut(String instanceId,JSONObject body, SecurityContext securityContext)
			throws NotFoundException {
		...
		if (serviceId == null || serviceId.equals("")) {
			...
			// JSON Body 转 DBAAS对象
			DaasServiceInstance dsi = TransformUtil.mapToObject(body,DaasServiceInstance.class);
			...
			return null;
			// return
			// Response.ok().entity(this.createJCSInstance(body)).build();
	 ...
	 */
	@Override
	public Response serviceInstancesInstanceIdPost(String instanceId,
			ServiceInstanceModify body, SecurityContext securityContext)
			throws NotFoundException {
		// 记录操作日志
	...
	private JsonObject createDBCSInstance(DaasServiceInstance body) {
		...
		Response response = myResource.request()
				.header(authorizationHeaderName, authorizationHeaderValue)
				.header("X-ID-TENANT-NAME", domain).post(Entity.json(body));
